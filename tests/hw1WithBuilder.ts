...
describe("User flow", function() {
  ...
  it("get information about user", async function() {
    ...
    expect(respUserInfo.body, JSON.stringify(respUserInfo.body))
      .to.be.an("object")
      .to.include.keys("_id", "profile");
    expect(typeof respUserInfo.body._id, respUserInfo.body._id).to.equal(
      "string"
    );
  });
  ...
  it("user logged-in", async function() {
    ...
    expect(
      typeof respUserLoggedin.body.profile,
      respUserLoggedin.body.profile
    ).to.not.equal("string");
    expect(
      typeof respUserLoggedin.body._id,
      respUserLoggedin.body._id
    ).to.equal("string");
  });
  ...
  it("delete user", async function() {
    ...
    expect(
      typeof respDeleteUser.body._id,
      JSON.stringify(respDeleteUser.body._id)
    ).to.equal("string");
    expect(
      respDeleteUser.body,
      JSON.stringify(respDeleteUser.body)
    ).to.include.keys("_id");
    expect(
      respDeleteUser.body._id,
      JSON.stringify(respDeleteUser.body._id)
    ).to.equal(respNewUser.body.id);
  });
