...
describe("User flow", function() {
  ...
  it("delete user", async function() {
    ...
    expect(
      typeof respDeleteUser._id,
      JSON.stringify(respDeleteUser._id)
    ).to.equal("string");
    expect(respDeleteUser, JSON.stringify(respDeleteUser)).to.include.keys(
      "_id"
    );
    expect(respDeleteUser._id, JSON.stringify(respDeleteUser._id)).to.equal(
      respNewUser.id
    );
  });
  ...
  it("users list", async function() {
    ...
    expect(respUsersList[0], JSON.stringify(respUsersList[0])).to.include.keys(
      "_id",
      "username"
    );
  });
  ...
  it("user logged-in", async function() {
    ...
    expect(
      typeof respUserLoggedin.profile,
      respUserLoggedin.profile
    ).to.not.equal("string");
    expect(typeof respUserLoggedin._id, respUserLoggedin._id).to.equal(
      "string"
    );
  });
});
